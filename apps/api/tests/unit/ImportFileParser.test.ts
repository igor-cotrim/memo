...
describe('ImportFileParser', () => {
  ...
  describe('parseJsonDeckImport', () => {
    ...
    it('throws ValidationError for invalid JSON', () => {
      expect(() => parseJsonDeckImport(Buffer.from('invalid json'))).toThrow(ValidationError);
    });
    ...
    it('throws ValidationError on prototype pollution', () => {
      ...
      expect(() => parseJsonDeckImport(Buffer.from(json))).toThrow('Invalid JSON file');
    });
  });
  ...
  describe('parseJsonCardsImport', () => {
    ...
    it('throws ValidationError for invalid JSON', () => {
      expect(() => parseJsonCardsImport(Buffer.from('invalid'))).toThrow(ValidationError);
    });

    it('throws ValidationError if not an array', () => {
      expect(() => parseJsonCardsImport(Buffer.from('{}'))).toThrow(/JSON must be an array/);
    });

    it('throws ValidationError if array is empty', () => {
      expect(() => parseJsonCardsImport(Buffer.from('[]'))).toThrow(/JSON array is empty/);
    });
  });
