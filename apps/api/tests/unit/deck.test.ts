...
describe("Deck Use Cases", () => {
  ...
  describe("CreateDeckUseCase", () => {
    ...
    it("should throw ValidationError for empty name", async () => {
      const useCase = new CreateDeckUseCase(deckRepo);
      await expect(useCase.execute(userId, { name: "" })).rejects.toThrow(
        ValidationError,
      );
    });
  });
  ...
  describe("GetDeckUseCase", () => {
    ...
    it("should throw ForbiddenError for non-owner", async () => {
      ...
      const get = new GetDeckUseCase(deckRepo);
      await expect(get.execute("user-2", deck.id)).rejects.toThrow(
        ForbiddenError,
      );
    });
    ...
    it("should throw NotFoundError for invalid id", async () => {
      const get = new GetDeckUseCase(deckRepo);
      await expect(get.execute(userId, "nonexistent")).rejects.toThrow(
        NotFoundError,
      );
    });
  });
