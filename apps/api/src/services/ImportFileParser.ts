...
function validateCardRows(
  rawCards: unknown[],
): { cards: ImportCardRow[]; errors: ImportRowError[] } {
  ...
  for (let i = 0; i < rawCards.length; i++) {
    ...
    const front =
      typeof row.front === "string" ? sanitizeField(row.front) : "";
    const back =
      typeof row.back === "string" ? sanitizeField(row.back) : "";
    const notes =
      typeof row.notes === "string" ? sanitizeField(row.notes) : undefined;
    ...
    if (front.length > MAX_FIELD_LENGTH || back.length > MAX_FIELD_LENGTH) {
      errors.push({ row: rowNum, message: `Field exceeds max length of ${MAX_FIELD_LENGTH} characters` });
      continue;
    }
    if (notes && notes.length > MAX_FIELD_LENGTH) {
      errors.push({ row: rowNum, message: `Notes field exceeds max length of ${MAX_FIELD_LENGTH} characters` });
      continue;
    }
...
export function parseJsonDeckImport(buffer: Buffer): ParsedDeckImport {
  ...
  if (!data || typeof data !== "object" || Array.isArray(data)) {
    throw new ValidationError(
      "JSON must be an object with 'name' and 'cards' fields",
    );
  }
  ...
  const description =
    typeof obj.description === "string"
      ? sanitizeField(obj.description) || undefined
      : undefined;
