import type { ReviewStats, DailyReviewCount, DeckAccuracy } from '@flashcard-app/shared-types';
...
export class GetReviewStatsUseCase {
  ...
  async execute(userId: string, timezoneOffset: number = 0): Promise<ReviewStats> {
    const now = new Date();

    // Last 365 days
    const yearAgo = new Date(now);
    yearAgo.setDate(yearAgo.getDate() - 365);
    const allLogs = await this.reviewLogRepo.findByUserId(userId, yearAgo.toISOString());
    ...
    for (const deck of decks) {
      const deckLogs = await this.reviewLogRepo.findByDeckId(deck.id, thirtyDaysAgo.toISOString());
    ...
    const reviewDates = await this.reviewLogRepo.getReviewDates(userId, timezoneOffset);
  ...
  private calculateStreak(dates: string[], now: Date, timezoneOffset: number): number {
