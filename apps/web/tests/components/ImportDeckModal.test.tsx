...
describe('ImportDeckModal', () => {
  ...
  const setup = () => {
    return render(<ImportDeckModal onClose={mockOnClose} onSuccess={mockOnSuccess} />);
  };

  it('renders the modal and its content', () => {
    setup();
    expect(screen.getByRole('heading', { name: 'decks.importModalTitle' })).toBeInTheDocument();
  ...
  it('handles valid CSV file drop and shows metadata fields', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByLabelText('decks.descriptionLabel')).toBeInTheDocument();
    });
