...
describe('ReviewCard', () => {
  it('renders the front of the card', () => {
    renderWithProviders(<ReviewCard card={baseCard} isFlipped={false} onFlip={vi.fn()} />);

    expect(screen.getByText('What is TypeScript?')).toBeInTheDocument();
  });

  it('renders the back of the card when flipped', () => {
    renderWithProviders(<ReviewCard card={baseCard} isFlipped={true} onFlip={vi.fn()} />);
  ...
  it('does not render notes when null', () => {
    ...
    renderWithProviders(<ReviewCard card={card} isFlipped={true} onFlip={vi.fn()} />);
  ...
  it('calls onFlip when clicked', async () => {
    ...
    renderWithProviders(<ReviewCard card={baseCard} isFlipped={false} onFlip={onFlip} />);
